...
import time
import sys
from core.llm_agent import complete_prompt, generate_py, detective
from Debug.fuzz_tester import run_fuzz_test
from Debug.executor import compile_and_debug, error_solving

def game_auto_creating(user_prompt: str):
    # 1. Optimize prompt
    ...
    # 2. Generate and save code (multi agents)
    filepaths, accumulated_skeleton = generate_py(user_prompt)
    main_game = filepaths[-1]           #main.py

    # 3. Execution and auto-repair loop 
    max_attempts = 4  # Set maximum number of detection attempts (3 times)
    wrong = True      
    ...
    for current_attempt in range(1, max_attempts + 1):
        ...
        exec_result = compile_and_debug(main_game)
        
        if not exec_result["state"]:
            # --- Failure Handling ---
            if current_attempt < max_attempts:
                print(f"🔧 [Executor] Execution failed. Performing repair attempt #{current_attempt}...")
                breaking_file = detective(exec_result["Text"], filepaths = filepaths)
                error_solving(exec_result["Text"], breaking_file = breaking_file, skeleton = accumulated_skeleton)
        ...
        fuzz_result = run_fuzz_test(main_game)
        ...
        if fuzz_result["state"]:
            ...
            # --- Failure Handling ---
            if current_attempt < max_attempts:
                print(f"🔧 [Fuzzer] Test failed. Performing logic repair attempt #{current_attempt}/{max_attempts}...")
                breaking_file = detective(fuzz_result["Text"], filepaths = filepaths)
                error_solving(fuzz_result["Text"], breaking_file = breaking_file, skeleton = accumulated_skeleton)
                continue
            else:
...
if __name__ == "__main__":
    ...
    user_request = input("Please enter the game you want to create (e.g., Snake): ")
    if user_request:
        game_auto_creating(user_request)