...
class MainMenuState(GameStateBase):
    ...
    def _setup_ui(self):
        ...
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y, btn_width, btn_height), "???å§???????", self.button_font,
            lambda: self.game.change_state(GameState.GAME_PLAYING, reset=True)))
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y + btn_height + 20, btn_width, btn_height), "è¦????", self.button_font,
            lambda: self.game.change_state(GameState.RULES)))
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y + 2 * (btn_height + 20), btn_width, btn_height), "??¢é????????", self.button_font,
            self.game.quit_game))
    ...
    def draw(self, surface):
        ...
        title_surf = self.title_font.render("Roguelike è¿·å®®??¢é??", True, (255, 255, 255))
...
class RulesState(GameStateBase):
    ...
    def _setup_ui(self):
        ...
        self.back_button = Button(
            (center_x - btn_width // 2, bottom_y, btn_width, btn_height), "è¿????ä¸»é?¸å??", self.button_font,
    ...
    def draw(self, surface):
        ...
        title_surf = self.title_font.render("?????²è?????", True, (255, 255, 255))
        title_rect = title_surf.get_rect(center=(SCREEN_WIDTH // 2, 50))
        surface.blit(title_surf, title_rect)

        rules_text = [
            "1. ??©å®¶??®æ????¯å??è¿·å®®???èµ·é?? (ç¶???²æ?¹å??) å®???¨æ?µé??çµ?é»? (ç´???²æ?¹å??) ä»¥å??å¾??????©ã??",
            "2. æ¯?æ¬¡é????²å????????ï¼?è¿·å®®?????¨æ????????ï¼?ä¸¦ä??è­???????è·¯å????½æ?¯é???????????",
            "3. ??©å®¶è¦??????????ï¼??????½ç????°è?ªèº«??¨å?? 3 ??¼ç???????§ç???????? (??°ç?­è¿·???)???",
            "   å·²æ?¢ç´¢ä½?ä¸???¯è??????????????è®????é¡¯ç¤ºï¼???ªæ?¢ç´¢?????????å®???¨é?±è?????",
            "4. è¿·å®®ä¸­é?¨æ?????ä½?å¯¶ç®± (é»???²æ?¹å??)ï¼???©å®¶??¾å??å¾???¯ç?²å???????½æ?¢å¾©????????¸å?????ç­?å¢?????????????",
            "5. è¿·å®®ä¸­é?¨æ?????ä½???·é?? (ç´???²æ?¹å??)ï¼???©å®¶è§¸ç?¼å??????????°å?·å®³???è²???¢ç?????å½±é?¿ã??",
            "6. ??©å®¶?????½å?¼æ­¸??¶æ??ï¼??????²å¤±??????",
            "7. ???ä¸? 'P' ??? 'ESC' ??µå?¯æ?«å???????²ã??",
            "8. ç§»å??: W/A/S/D ??? ??¹å????µã??",
        ]
...
class GamePlayingState(GameStateBase):
    ...
    def reset_game(self):
        ...
        if self.player:
            ...
            # ???ä¿®æ­£???è£¡ã???????°é??å§????ï¼??????¶ç?©ä»¶?????¨ï??ä½??????ºä????? empty() ???äº?ï¼?å¿?????????°å????¥ç¾¤çµ?ï¼?
            self.all_sprites.add(self.player)   # å»ºè­°?????? all_sprites
            self.camera_group.add(self.player)  # å¿?????????? camera_group ?????½è¢«??«å?ºä??
        else:
    ...
    def _handle_chest_collision(self, player, chest):
        if chest.on_collect(player):
            ...
            self.camera_group.remove(chest) # ???ä¿®æ­£ 5???å¾???«é?¢ä¸­ç§»é??
            self.chests_pool.release(chest)
            self.floating_texts.append(FloatingText(
                "å¯¶ç®±! HP+1", (player.rect.centerx, player.rect.top - 20), 
    ...
    def _handle_trap_collision(self, player, trap):
        if trap.on_trigger(player):
            ...
            self.camera_group.remove(trap) # ???ä¿®æ­£ 6???å¾???«é?¢ä¸­ç§»é??
            self.traps_pool.release(trap)
            self.floating_texts.append(FloatingText(
                "??·é??! HP-1", (player.rect.centerx, player.rect.top - 20), 
    ...
    def _draw_hud(self, surface):
        ...
        hp_surf = self.hud_font.render(hp_text, True, (255, 50, 50)) # ç´????
        
        # é¡¯ç¤ºå¯¶ç®±??? (é»????)
        chest_text = f"Chests: {self.player.collected_chests}"
        chest_surf = self.hud_font.render(chest_text, True, (255, 255, 0)) # é»????

        # 2. ??«å?¨ç?«é?¢ä?? (å·¦ä??è§?)
        # HP ??«å?? (10, 10)
        surface.blit(hp_surf, (10, 10))
        
        # å¯¶ç®±??¸ç?«å?? HP ä¸???¢ä??é»?é»? (10, 40)
...
class PauseMenuState(GameStateBase):
    ...
    def _setup_ui(self):
        ...
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y, btn_width, btn_height), "ç¹¼ç????????", self.button_font,
            lambda: self.game.change_state(GameState.GAME_PLAYING)))
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y + btn_height + 20, btn_width, btn_height), "?????°é??å§?", self.button_font,
            lambda: self.game.change_state(GameState.GAME_PLAYING, reset=True))) # Restart immediately to game playing
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y + 2 * (btn_height + 20), btn_width, btn_height), "è¦????", self.button_font,
            lambda: self.game.change_state(GameState.RULES)))
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y + 3 * (btn_height + 20), btn_width, btn_height), "???ä¸»é?¸å??", self.button_font,
            lambda: self.game.change_state(GameState.MAIN_MENU)))
    ...
    def draw(self, surface):
        ...
        title_surf = self.title_font.render("?????²æ?«å??", True, (255, 255, 255))
        title_rect = title_surf.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 4))
        surface.blit(title_surf, title_rect)
...
class GameOverState(GameStateBase):
    ...
    def _setup_ui(self):
        ...
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y, btn_width, btn_height), "?????°é??å§?", self.button_font,
            lambda: self.game.change_state(GameState.GAME_PLAYING, reset=True)))
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y + btn_height + 20, btn_width, btn_height), "???ä¸»é?¸å??", self.button_font,
            lambda: self.game.change_state(GameState.MAIN_MENU)))
        self.buttons.append(Button(
            (center_x - btn_width // 2, start_y + 2 * (btn_height + 20), btn_width, btn_height), "??¢é????????", self.button_font,
            self.game.quit_game))
    ...
    def draw(self, surface):
        ...
        title_text = "??­å???????©ï??" if self.outcome == "WIN" else "?????²å¤±???ï¼?"
        ...
        stats_text = f"??¾å??å¯¶ç®±???: {self.collected_chests}"
        ...
        if self.outcome == "LOSE":
            hp_text = f"??©é???????½å??: {max(0, self.player_health)}"
...
plan_json = """
{
  ...
  "technical_architecture": {
    ...
    "implementation_details": "?????²æ?¸å??ä½¿ç?? `tile_map.py` ??²è??è¿·å®®???????????ºç????¦ç??ç®¡ç???????©å®¶???å¯¶ç®±?????·é?±ç??äº????å¯¦é????ºæ?? `sprite_manager.py` ???ä¾???? `GameSprite` é¡???¥å»ºç«????å¯¦é????????äº???????ç¢°æ????µæ¸¬??? `collision.py` é«???????????????¸æ??ç³»çµ±ä»? `camera_player_center.py` ??ºå?ºç??ï¼????ä¾???©å®¶ä¸­å??è¦?è§????ç²¾é?? Y-Sort æ¸²æ??ï¼?ä½???¶è????¯ç¹ªè£½é??è¼¯å??å®¢è£½???ä»¥å¯¦ä½???°ç?­è¿·??? (Fog of War) ?????½ï????????æ¸²æ????¯è?????å·²æ?¢ç´¢?????????è¿·å®®??¦ç??????????²ç????´é??æµ?ç¨??????????è½????å°?????????ªå??ç¾©ç??????????????æ©? (FSM) å¯¦ç?¾ï???????«ä¸»??¸å?®ã???????²é?²è???????«å?????è¦????????????©å??å¤±æ??ç­??????????"
  },
  "game_rules": [
    "??©å®¶??®æ????¯å??è¿·å®®???èµ·é?? (ç¶???²æ?¹å??) å®???¨æ?µé??çµ?é»? (ç´???²æ?¹å??) ä»¥å??å¾??????©ã??",
    "æ¯?æ¬¡é????²å????????ï¼?è¿·å®®?????¨æ????????ï¼?ä¸¦ä??è­???????è·¯å????½æ?¯é???????????",
    "??©å®¶è¦??????????ï¼??????½ç????°è?ªèº«??¨å?? 3 ??¼ç???????§ç???????? (??°ç?­è¿·???)???å·²æ?¢ç´¢ä½?ä¸???¯è??????????????è®????é¡¯ç¤ºï¼???ªæ?¢ç´¢?????????å®???¨é?±è?????",
    "è¿·å®®ä¸­é?¨æ?????ä½?å¯¶ç®± (é»???²æ?¹å??)ï¼???©å®¶??¾å??å¾???¯ç?²å???????½æ?¢å¾©????????¸å?????ç­?å¢?????????????",
    "è¿·å®®ä¸­é?¨æ?????ä½???·é?? (ç´???²æ?¹å??)ï¼???©å®¶è§¸ç?¼å??????????°å?·å®³???è²???¢ç?????å½±é?¿ã??",
    "??©å®¶?????½å?¼æ­¸??¶æ??ï¼??????²å¤±??????",
    "???ä¸? 'P' ??? 'ESC' ??µå?¯æ?«å???????²ï????²å?¥æ?«å????¸å?®ï?????ä¾?ç¹¼ç??????????°é??å§????è¦?????????¢é????¸é?????",
    "?????²ç????? (?????©æ??å¤±æ??) å¾?ï¼????é¡¯ç¤ºçµ?ç®???«é?¢ä¸¦???ä¾??????°é??å§??????¢é???????²ç????¸é?????"
  ],
  "entities": [
    {
      "name": "Player",
