...
    7. In which compute environment are you running?

        Selection: This machine

  

    8. Which type of machine are you using?

        Selection: No distributed training

  

    9. Do you want to run deterministic algorithms?

        Selection: No

  

    10. Do you wish to optimize your script with torch dynamo?

        Selection: No

  

    11. Do you want to use DeepSpeed?

        Selection: No

  

    12. What GPU(s) (by id) should be used?

        Selection: all

  

    13. Would you like to enable numa efficiency? (Currently only supported on NVIDIA hardware). [yes/NO]:

        Selection: IF your GPU is NVIDIA then yes, otherwise choose NO

    14. Do you wish to use FP16 or BF16 (mixed precision)?
...
15. How to open the kohya_ss website

    16. Enter .\gui.bat in the terminal

    17. Find the line written " Running on local URL:  http://127.0.0.1:7860", the http is the website
...
### 4. Art Diffusion model(`art_diffusion_model/`)
- `Base_models/` and `models` : Store the base SDXL models and LoRA models separately, they are both hidden because of capacity, you can download the models base on the http link or you can train your own models
- `graph_creator.py`: Generate the images needed by the game, and store the images into the directory which you can choose by yourself

### 5. Dynamic Testing & Debugging (`Debug/`)
...
* `tools.py`: Storage utility functions include cleaning up Markdown syntax generated by LLM, outputting the cleaned code as a Python file, and retrieving variables from JSON format.
...
# Future Plans
改進hitbox問題，以及增加更多RAG，添加front-end介面(JavaScript)