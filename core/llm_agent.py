...
def generate_py(user_prompt: str):
    ...
    system_instruction_planner = (
        ...
        "\n- Exact Match & Spacing: Filenames MUST perfectly match the generated asset list. NO leading/trailing spaces (e.g., [sprite]mage.png)."
        ...
        "\n- Dynamic Image Scaling (CRITICAL): Assume ALL generated visual assets have a base resolution of 1024x1024. You MUST logically calculate and assign an IMAGE_SCALE float value for EVERY entity, UI element, and tile based on the SCREEN_SIZE and the entity's role."
        "\n  - Example Logic: If screen is 1280x720, a Player sprite should be around 80px (IMAGE_SCALE: 0.08), a massive Boss around 250px (IMAGE_SCALE: 0.25), a tiny bullet around 20px (IMAGE_SCALE: 0.02), and a background needs to fill the screen (IMAGE_SCALE: 1.25)."
        "\n- Ensure filenames match the [category] name.png format exactly."
        "\nFollow the Schema below."
        "\n```json"
        "\n{"
        "\n  \"game_name\": \"...\", "
        "\n  \"config\": {\"FPS\": 60, \"SCREEN_SIZE\": [1280, 720]},"
        "\n  \"entities\": [ { \"name\": \"Player\", \"image\": \"[sprite]mage.png\", ... } ]"
        "\n}"
        "\n```"
        ...
        "\n- Every asset mentioned MUST include either the `[sprite]` or `[background]` tag."
    )
    ...
    system_game_designer = (
        ...
        "You MUST follow strict coding standards to prevent common runtime errors (TypeError, NameError, KeyError). "
        "No Markdown formatting in output."
        "Before writing the code, you MUST output a <THINKING> block. In this block, explicitly list every mechanic requested in the Design Document (e.g., Invulnerability frames, Score tracking, specific collision matrices) and state exactly which class and method will implement them. Only after this block, output the complete Python code."

        "\n\n### 1. ARCHITECTURE & CONSTRUCTOR CONTRACT (Prevent TypeError)"
        "\n- **Signature Matching**: ALL classes inheriting from `GameSprite` MUST use this exact signature: "
        "\n  `def __init__(self, [custom_params], pool=None, **kwargs):`"
        "\n- **Super Inheritance**: You MUST call `super().__init__(pool=pool, **kwargs)` as the first line of any `__init__` override."
        "\n- **ObjectPool Compatibility**: Ensure that any class passed to `ObjectPool` can handle `pool=self` being passed into its constructor."

        "\n\n### 2. INITIALIZATION & FSM SEQUENCE (Prevent KeyError)"
        "\n- **Lazy FSM**: The `FSM` class MUST NOT call `self.change()` inside its `__init__`."
        "\n- **Registration First**: In the `Game` class, you MUST use `fsm.add()` to register ALL states (MainMenu, Playing, Rules, etc.) BEFORE calling the first `fsm.change()`."
        "\n- **Zero-Argument Init**: `Game.__init__(self)` MUST NOT require arguments. All setup must be internal or via JSON."

        "\n\n### 3. DATA SAFETY & DEFAULTS (Prevent NameError & NoneType)"
        "\n- **Safe Property Access**: When fetching from `GameConfig` or JSON, ALWAYS provide a numeric default value: "
        "\n  `value = config.get_prop('Entity', 'Key', default_value)`. NEVER allow a return of `None` for math operations."
        "\n- **Defensive Iteration**: When looping over optional lists (like sprite groups), use: "
        "\n  `groups = kwargs.get('groups') or []` to prevent 'NoneType object is not iterable' errors."
        "\n- **Variable Scope**: Ensure all managers (Camera, UI, Particles) are initialized as `self.attribute` in `Game.__init__` before being used in states."
        ...
        "\n- **Smoothness**: Implement acceleration/friction and 'Corner Nudging' to prevent getting stuck in 1:1 tile corridors."
        "\n- **Juice Protocol**: Apply 0.1s Input Buffer, Coyote Time, and Screen Shake. Hitboxes MUST be 20% smaller than sprites."

        "\n\n### 5. UI, CAMERA & MENUS"
        "\n- **Camera Spawning**: Upon level load, IMMEDIATELY call `camera.center_on_target()` to prevent the player spawning off-screen."
        "\n- **Startup Menu (3-Button)**: MUST have '遊戲開始', '規則', '結束'."
        "\n- **Pause Menu (4-Button)**: Triggered by P/ESC. MUST have '繼續遊戲', '重新遊戲', '結束遊戲', '規則'."
        "\n- **Rules State**: MUST implement a dedicated state/overlay to display controls and mechanics."
        "\n- UI Delegation: Any UIManager or similar controller MUST implement a handle_event(event) method that iterates and calls handle_event() on all active UI elements (e.g., Buttons). NEVER leave an event handler dangling in the FSM."

        "\n\n### 6. ASSET MANAGEMENT"
        "\n- **Exact Pathing**: Use `os.path.join(os.path.dirname(__file__), 'assets', filename)` with `.convert_alpha()`."
        "\n- **No Prefixes**: DO NOT add 'Graphic/' or other subfolders. Use the filenames EXACTLY as listed."
        f"\n- **Filenames**: {available_assets_str}"

        "\n\n### 7. EXECUTION"
        "\nParse JSON logic -> Apply Constructor Contract -> Apply Data Safety -> Output complete Python code."

        "\n\n### 8. CRITICAL ERROR PREVENTION (STRICT)"
        "\n- Target JSON: You MUST load the config exactly from game_config.json. DO NOT use param.json or params.json, which will cause FileNotFoundError."
        "\n- Dynamic Image Scaling: You MUST retrieve the IMAGE_SCALE property from the JSON config for EVERY asset (Entities, Tiles, UI elements, Buttons). You MUST apply pygame.transform.smoothscale() (for UI/Backgrounds) or pygame.transform.scale() (for Sprites) inside their __init__ methods based on this specific JSON value. Do not hardcode scale values."
        "\n- Null-Safety / AttributeError Prevention: NEVER assume dictionary lookups succeed. When fetching entity data like player_data = config.get_entity('Player'), you MUST append or {} to prevent AttributeError: 'NoneType' object has no attribute 'get' during subsequent .get() calls."
        "\n- String Matching: Use .strip() and .lower() when comparing asset names or entity names from JSON to avoid hidden space or case-sensitivity bugs."
        "\n- You MUST NOT hardcode entity_data for ANY class instance (including UI Buttons). You MUST fetch it via config.get_entity('Exact_Name'). Any hardcoded dictionary in an __init__ call is a failure."
    
        "\n ### 9.【CRITICAL UI RENDERING RULE】\n"
        "1. The AI-generated images for buttons and UI panels are BLANK FRAMES. They do NOT contain text.\n"
        "2. YOU MUST write Python code in your UI classes (e.g., `Button` class) to dynamically render text on top of these images.\n"
        "3. Fetch the text string from the JSON configuration using `entity_data.get('TEXT_OVERLAY', 'Button')`.\n"
        "4. Initialize a font in the `__init__` method: `self.font = pygame.font.Font(None, 36)`.\n"
        "5. In the `draw` method, AFTER blitting the button's image, render the text using `self.font.render` and center it perfectly by setting the text's rect center to the button's rect center (`text_rect.center = self.rect.center`).\n"
        "6. Optional but recommended: Draw a dark text shadow slightly offset for better visibility before drawing the main white text.\n"
    )
...
def art_director_plan_assets(design_doc: str) -> list:
    ...
    art_prompt = (
        ...
        "CRITICAL CHROMA KEY RULE: For EVERY [sprite] asset, you MUST append 'pure green background' to its 'pos_prompt' to act as a chroma key for the AI background remover.\n"
        ...
        "- Add 'blank, empty, no text, UI frame' to the 'pos_prompt'.\n"
        "- You MUST ADD 'text, letters, words, typography, watermark, signature' to the 'neg_prompt' to strictly prevent the AI from generating gibberish text.\n"
        ...
        # 👇 範例也配合改成純綠色背景 👇
        "    \"pos_prompt\": \"pixel art spaceship, silver, pure green background\",\n"
